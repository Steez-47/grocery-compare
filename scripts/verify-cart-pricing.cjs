...
app.whenReady().then(async()=>{
 ...
 const amounts=()=>js(`({line:document.querySelector('.line-bottom > strong').textContent,total:document.querySelector('.total strong').textContent,quantity:document.querySelector('.stepper span').textContent,blocked:document.querySelector('.checkout').disabled})`);
 ...
 assert.deepEqual(await amounts(),{line:'$2.28',total:'$2.28',quantity:'0.7 kg',blocked:false});
 checks.push('Weighted half-cent line price matches subtotal in the built renderer');
 await js(`document.querySelector('[aria-label="More Test apples"]').click()`);
 await until(`document.querySelector('.stepper span').textContent==='0.8 kg'`);
 assert.equal((await amounts()).total,'$2.60');
 await js(`document.querySelector('[aria-label="More Test apples"]').click()`);
 await until(`document.querySelector('.stepper span').textContent==='0.9 kg'`);
 assert(await js(`document.querySelector('[aria-label="More Test apples"]').disabled`));
 await js(`document.querySelector('[aria-label="Less Test apples"]').click()`);
 await until(`document.querySelector('.stepper span').textContent==='0.8 kg'`);
 ...
 await load(free);assert.deepEqual(await amounts(),{line:'$0.00',total:'$0.00',quantity:'0.7 kg',blocked:false});
 ...
 assert.equal((await amounts()).quantity,'0.7 kg');
 assert(await js(`document.querySelector('.shopping-mode p').textContent.includes('keeping the item choices')`));
 ...
 await until(`Boolean(document.querySelector('.recommendation-shelf .add-button'))`);
 await js(`document.querySelector('.recommendation-shelf .add-button').click()`);
 ...
 assert(await js(`document.querySelector('.basket-empty').textContent.includes('Use + on a product')`));
