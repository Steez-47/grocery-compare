...
app.whenReady().then(async()=>{
 ...
 handle('search',async(r,store,q,page=0,force=false)=>{if(!['newworld','woolworths'].includes(r)||typeof q!=='string'||q.length>100||!Number.isInteger(page)||page<0||page>28||!store?.id||!/^[a-zA-Z0-9-]+$/.test(store.id))throw new Error('Invalid search.');return cat.search(r,store,q,page,force);});
