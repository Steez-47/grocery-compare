...
app.whenReady().then(async()=>{
 ...
 const {repairWeightProduct}=require('./model.cjs');state.basket=state.basket.map(l=>({...l,product:repairWeightProduct(l.product),offers:Object.fromEntries(Object.entries(l.offers).map(([r,p])=>[r,repairWeightProduct(p)]))}));
