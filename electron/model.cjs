...
function groupProducts(products){
 ...
 unique.forEach((p,i)=>{if(used.has(i))return;const row={key:p.retailer+':'+p.id,product:p,offers:{[p.retailer]:p}},j=best[i];used.add(i);if(j>=0&&best[j]===i&&!used.has(j)){row.offers[unique[j].retailer]=unique[j];row.equivalent=clean(p.brand)!==clean(unique[j].brand);used.add(j)}rows.push(row)});
