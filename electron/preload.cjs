...
contextBridge.exposeInMainWorld('grocery',{
 windowState:()=>invoke('window-state'),windowControl:action=>invoke('window-control',action),
 onWindowState:cb=>{const f=(_e,s)=>cb(s);ipcRenderer.on('window-state',f);return()=>ipcRenderer.removeListener('window-state',f);},
 appearance:()=>invoke('appearance'),setAppearance:v=>invoke('appearance-set',v),browserStatus:()=>invoke('browser-status'),connectBrowser:()=>invoke('browser-connect'),disconnectBrowser:()=>invoke('browser-disconnect'),extensionFolder:()=>invoke('browser-folder'),
 ...
 homeCandidates:(id,stores)=>invoke('home-candidates',id,stores),homeRank:(products,deals,limit,loyalty,basket)=>invoke('home-rank',products,deals,limit,loyalty,basket),
