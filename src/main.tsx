...
function App(){
 ...
 {!state.basket.length?<div className="basket-empty"><ShoppingBag size={34} strokeWidth={1.4}/><p>Your basket is empty</p></div>:<><div className="basket-lines">{state.basket.map(line=>{const offer=model.selectOffer(line,state.policy,state.loyalty) as Product|null;const rules=model.quantityRules(line,offer||line.offers[line.preferred as Retailer]||line.product);return <div className="basket-line" key={line.key}><Picture p={offer||line.product}/><div className="line-body"><div className="line-title"><h3>{offer?.name||line.product.name}</h3><button className="icon-button" aria-label={`Remove ${line.product.name}`} onClick={()=>setState(s=>({...s,basket:s.basket.filter(l=>l.key!==line.key)}))}><X size={14}/></button></div><div className="line-store"><select aria-label={`Store for ${line.product.name}`} value={line.preferred} onChange={e=>setState(s=>({...s,basket:s.basket.map(l=>l.key===line.key?{...l,preferred:e.target.value as Policy,quantity:e.target.value==='cheapest'?l.quantity:model.fitQuantity(l.quantity,model.quantityRules(l,l.offers[e.target.value as Retailer]!))}:l)}))}><option value="cheapest">Cheapest</option>{retailers.filter(r=>line.offers[r]).map(r=><option value={r} key={r}>{names[r]}</option>)}</select><button className="text-button" onClick={()=>setMatch(line)}>Compare</button></div><div className="line-bottom"><div className="stepper"><button aria-label={`Less ${line.product.name}`} onClick={()=>changeQty(line.key,line.quantity-rules.step<rules.min?-line.quantity:-rules.step)}><Minus size={13}/></button><span>{line.quantity}{line.product.unit==='kg'?' kg':''}</span><button aria-label={`More ${line.product.name}`} disabled={line.quantity+rules.step>rules.max} onClick={()=>changeQty(line.key,rules.step)}><Plus size={13}/></button></div><strong>{offer?dollars(Math.round(effectivePrice(offer,state.loyalty[offer.retailer])!*line.quantity)):'Unavailable'}</strong></div></div></div>})}</div>
 <div className="basket-bottom"><div className="plan-label"><span>Where to shop</span><button className="text-button" onClick={()=>refreshBasket()} disabled={refreshing}><RefreshCw size={13} className={refreshing?'spin':''}/> Refresh</button></div><div className="plans">{choices.map(p=>{const total=basketSummary(state.basket,p,state.loyalty);return <button key={p} className={'plan '+(state.policy===p?'chosen':'')} onClick={()=>setState(s=>({...s,policy:p,basket:p==='cheapest'?s.basket:s.basket.map(l=>l.offers[p]?.unit==='kg'?{...l,quantity:model.fitQuantity(l.quantity,model.quantityRules(l,l.offers[p]))}:l)}))}><span>{p==='cheapest'?'Split shop':names[p]}</span><strong>{dollars(total.cents)}</strong>{total.missing>0&&<small>{total.missing} missing</small>}</button>})}</div>{stale&&<div className="stale">Prices need refreshing <button onClick={()=>refreshBasket()} disabled={refreshing}>Refresh</button></div>}<div className="total"><span>Subtotal</span><strong>{dollars(summary.cents)}</strong></div><div className="subtotal-note">{summary.missing?`${summary.missing} items unavailable · `:''}Before delivery and bag fees</div><button className="primary checkout" disabled={refreshing||stale||summary.missing>0} onClick={()=>setCheckout(true)}>Continue to stores <ArrowRight size={17}/></button></div></>}
 </aside></div>
