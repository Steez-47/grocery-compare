...
import {ChevronDown,Plus,ShoppingBag,Check,Tag} from 'lucide-react';
...
function Sale({product,member}:{product:Product;member:boolean}){
 const deal=pricing.saleInfo(product,member);if(!deal)return null;
 return <span className="sale-details"><span className="sale-badge"><Tag size={11}/>{deal.label}{deal.percent>0?` · ${deal.percent}% off`:''}</span>{deal.reference!==null&&<small>{deal.referenceLabel} <s>{money(deal.reference)}</s>{product.unit==='kg'?' / kg':' each'}</small>}</span>;
}
export function ProductCard({row,state,onAdd,onSimilar,onMatch,onSeparate,pending=false}:{pending?:boolean;row:Row;state:State;onAdd:(row:Row,r?:Retailer,quantity?:number)=>void;onSimilar?:(p:Product)=>void;onMatch?:(row:Row)=>void;onSeparate?:(row:Row)=>void}){
 ...
 return <article className="product-row">
  {selected&&<Sale product={selected} member={state.loyalty[selected.retailer]}/>}
  <div className="product-row-main"><div className="row-picture">{!failed&&product.image?<img src={product.image} alt="" loading="lazy" decoding="async" onError={()=>setFailed(true)}/>:<ShoppingBag size={27}/>}</div>
   ...
   <div className="row-buy"><Quantity value={quantity} rules={rules} name={product.name} onChange={setQuantity} onValidity={setValid}/><button className="primary row-add" disabled={pending||!selected||selected.restricted||!valid||atLimit} aria-label={`Add ${product.name} at cheapest price`} onClick={()=>onAdd(row,undefined,quantity)}><Plus size={16}/>{pending?'Checking…':product.restricted?'18+':atLimit?'At limit':'Add'}</button></div>
  </div>
  <details className="row-comparison"><summary>{pending?'Comparing prices…':available.length===2?'Compare 2 stores':selected?'1 store available':'Check store availability'}<ChevronDown size={13}/></summary><div className="comparison-content">{(['newworld','woolworths'] as Retailer[]).map(r=>{const p=row.offers[r];return <div className="comparison-offer" key={r}><span className={'store-badge '+r}>{r==='newworld'?'NW':'W'}</span><div><strong>{names[r]}</strong><small>{p?p.name:'No matching product found'}</small>{p&&<Sale product={p} member={state.loyalty[r]}/>}</div><span>{p&&p.available?money(model.linePrice(p,quantity,state.loyalty[r])):'Unavailable'}</span></div>})}<div className="comparison-actions">{onSimilar&&<button onClick={()=>onSimilar(product)}>Similar items</button>}{onMatch&&<button onClick={()=>onMatch(row)}>Find a match</button>}{paired&&onSeparate&&<button onClick={()=>onSeparate(row)}>These aren't the same</button>}</div></div></details>
 </article>;
}
