...
export interface GroceryAPI{
 homeCandidates:(id:string,stores:State['stores'])=>Promise<{products:Product[];partial:boolean}>;homeRank:(products:Product[],deals:boolean,limit:number,loyalty:State['loyalty'],basket:Product[])=>Promise<Row[]>;
 windowState:()=>Promise<{maximized:boolean}>;windowControl:(action:'minimize'|'maximize'|'close')=>Promise<boolean>;onWindowState:(cb:(s:{maximized:boolean})=>void)=>()=>void;
