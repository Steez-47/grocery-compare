...
Install `Grocery-Compare-Setup-0.1.0.exe` from this private repository's Releases page. Windows may ask you to confirm running the unsigned installer.

1. Choose your New World and Woolworths pickup locations.
2. Search. Products matched across the stores appear first, with both prices on one card.
...
4. Use **Compare** for equivalents the app did not automatically pair. Different brands remain separate until you choose an equivalent.
...
- Matching normalizes brand, units, pack counts and specific naming differences. It preserves variant distinctions and leaves ambiguous or size-less listings separate. Search results are paginated; use **Show more** to retrieve more products.
